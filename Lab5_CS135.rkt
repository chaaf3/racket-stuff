...
;; 3 PTS
(define (id n)
  (if (equal? n 0) '() (append (list(list n n)) (id (- n 1)))))
...
;; 3 PTS
(define (converse R)
  (if (null? R) '() (append (list(reverse (car R))) (converse (cdr R)))))
...
(define (helper R)
  (cond [(null? R) '()]
        [else (append (cdr (car R)) (helper (cdr R)))]))
;; Type signature: (injective? relation) -> boolean
;; 3 PTS
(define (injective? R)
  (if (equal? (helper R) (make-set (helper R))) #t #f ))
