...
(define (gcd a b)
  (cond
    ...
    [(equal? (modulo b a) 0) a]
    [(> a b) (gcd b (- a (*(quotient a b) b)))]
    [else (gcd a (- b (*(quotient b a) a)))])
  )
...
(define (wj-solvable? cA cB g)
  (if (> g cA) (if (> g cB) #f (divides? (gcd cA cB) g))(divides? (gcd cA cB) g)))
...
(define (waterjug cA cB g)
  (define (waterHelper fA fB storage)
    (cond
      [(equal? fB g) (reverse (cons (list fA fB) storage))]
      [(equal? fB cB) (waterHelper fA 0 (cons (list fA fB) storage))]
      [(equal? fA 0) (waterHelper cA fB (cons (list fA fB) storage))]
      [(> (+ fA fB) cB) (waterHelper (- fA (- cB fB)) cB (cons (list fA fB) storage))]
      [else (waterHelper 0 (+ fA fB) (cons (list fA fB) storage))])
    )
  (waterHelper 0 0 '())
  )
...
#| Implement "factorize" to accept a natural n
 ...
 |#
(define (factorize n)
  (define (factorizeHelper smallest k list)
    (cond
      [(equal? k 1) (reverse list)]
      [(equal? 0 (modulo k smallest)) (factorizeHelper smallest (/ k smallest) (cons smallest list))]
      [else (factorizeHelper (findNext smallest) k list)])
    )
  (cond
    [(equal? n 1) '(1)]
    [else (factorizeHelper 2 n '())])
  )
...
;; 3 PTS
(define ((cong-mod? m) a b)
  (define (cong-modHelper a b)
    (cond
      [(equal? (modulo a m) (modulo b m)) #t]
      [else #f])
    )
  (cong-modHelper a b))
