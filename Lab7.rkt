...
;; 1 PTS
(define (pred p)
  (if (pz? p) (error-predecessor) (car p)))
...
(define (p- a b)
  (cond
    [(pz? a) (if (pz? b) z (error-subtraction))]
    [(pz? b) a]
...
;; 2 PTS
(define (p* a b)
  (define (multHelper a1 b1)
    (cond
    [(pz? a) a]
    [(pz? b) b]
    [(pz? (pred a1)) b1]
    [else (multHelper (pred a1)(p+ b b1))]))
  (multHelper a b))
...
;; 1 PTS
(define (p^ a b)
  (define (powHelp a1 b1)
    (cond
    [(pz? a) a]
    [(pz? b) (succ b)]
    [(pz? (pred b1)) a1]
    [else (powHelp (p* a a1)(pred b1))]))
  (powHelp a b))
...
;; 3 PTS
(define (p-prime? p)
  (define (primeHelper n)
  (cond
    [(pz? p) #f]
    [(pz? (pred p)) #f]
    [(pz? (pred (pred p))) #t]
    [(p-divides? (pred n) p) #f]
    [(pz? (pred (pred (pred n)))) #t]
    [else (primeHelper (pred n))]
    )
  )
  (primeHelper p)
  )