...
;; 3 PTS
(define (sum-pow p n)
  (if (<= n 0) 0 (+(sum-pow p (- n 1)) (expt n p))))
...
(define (subfact n)
  (if (<= n 1) 0
      (if (= n 2) 1
          (if (= (remainder n 2) 0) (+(* n (subfact(- n 1))) 1) (-(* n (subfact(- n 1))) 1))
      )
  )
)
...
(define (gen-fib a b n)
  (if (<= n 0) a
      (if (= n 1) b (+(gen-fib a b (- n 1)) (gen-fib a b (- n 2))))))
...
;; 4 PTS
(define (filter P L)
  (if (null? L) '() (if (P (car L)) (cons (car L) (filter P (cdr L))) (filter P (cdr L)))))