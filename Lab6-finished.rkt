...
(define (helper L B)
  (cond [(null? L) '()]
        [else (cons (list B (car L)) (helper (cdr L) B))])
  )

(define (compose A B)
  (cond [(null? B) '()]
        [else (make-set(append (helper (relate (cadr (car B)) A) (caar B)) (compose A (cdr B))))])
  )
...
(define (power k R)
   (define (powHelp c G)
     (cond [(equal? c 0) G]
           [else (powHelp (- c 1) (compose R G))])
     )
  #| 
   (cond [(equal? k 0) )]
         [else (power (- k 1) (powHelp k R))])
|#
   (powHelp k R)
   )
...
(define (transitive-closure R)
  (define (tranHelp G reducer n)
     (cond [(null? reducer) G]
           [else (union G (tranHelp (power n R) (cdr reducer) (+ n 1)))]
           )
    )
  (tranHelp R R 0)
  )
...
(define (equivalence? n R)
  (if (reflexive? n R) (if (symmetric? R) (if (transitive? R) #t #f)#f)#f)
  )
